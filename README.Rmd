...
In order to initiate the calculations, the two cohorts tables need to be intersected using `CohortSymmetry::generateSequenceCohortSet()`. This process will output all the individuals who appeared on both tables according to a user-specified parameters. This includes `timeGap`, `washoutWindow`, `indexMarkerGap` and `daysPriorObservation`. Details on these parameters could be found on the vignette.
...
To get the sequence ratios, we would need the output of the generateSequenceCohortSet() function to be fed into `CohortSymmetry::summariseSequenceRatio()` 
...
res <- CohortSymmetry::summariseSequenceRatio(cohort = cdm$aspirin_amoxicillin)
