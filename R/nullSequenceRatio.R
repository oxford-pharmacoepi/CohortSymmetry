...
nullSequenceRatio <- function(table, movingAverageRestriction = 548) {
  ...
  # The case movingAverageRestriction is finite:
  if (is.finite(movingAverageRestriction)) {

    table <-
      table %>%
      dplyr::mutate(
        marker_cumsum_fwd = deltaCumulativeSum(.data$marker_first, .data$days_first, movingAverageRestriction, backwards = FALSE), # For each days_first, look back 548 (movingAverageRestriction) days and see how many marker_first are there
        marker_cumsum_bwd = deltaCumulativeSum(.data$marker_first, .data$days_first, movingAverageRestriction, backwards = TRUE), # For each days_first, look forward 548 (movingAverageRestriction days) and see how many marker_first are there
