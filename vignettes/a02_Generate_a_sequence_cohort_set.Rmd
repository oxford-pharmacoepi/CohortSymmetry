...
```{r message= FALSE, warning=FALSE}
 cdm <- generateSequenceCohortSet(
   ...
   indexTable = "aspirin",
   markerTable = "acetaminophen",
   name = "intersect_date_range",
   cohortDateRange = as.Date(c("1950-01-01","1969-01-01")),
   daysPriorObservation = 0,
   combinationWindow = c(0,Inf))

 cdm$intersect_date_range |>
   dplyr::inner_join(
...
```{r message= FALSE, warning=FALSE}
 cdm <- generateSequenceCohortSet(
   ...
   indexTable = "aspirin",
   markerTable = "acetaminophen",
   name = "intersect_prior_history",
   cohortDateRange = as.Date(c("1950-01-01","1980-01-01")),
   daysPriorObservation = 365,
   combinationWindow = c(0,Inf))
 
 cdm$intersect_prior_history |>
   dplyr::inner_join(
     cdm$observation_period |>
       dplyr::select("subject_id" = "person_id", "observation_period_start_date")
  ) |>
...
cdm <- generateSequenceCohortSet(
  ...
  indexTable = "aspirin",
  markerTable = "acetaminophen",
  name = "intersect_washout_365",
  cohortDateRange = as.Date(c("1950-01-01","1969-01-01")),
  daysPriorObservation = 365,
  washoutWindow = 365,
  combinationWindow = c(0, Inf))

cdm$intersect_washout_365 |>
  dplyr::filter(subject_id %in% c(1936,3565)) |>
