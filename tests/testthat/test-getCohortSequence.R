...
test_that("mock db: check output table name", {
  ...
  expect_true(all(
    c("index_id", "marker_id", "subject_id", "index_date", "marker_date", "first_date", "second_date", "days_prior_observation", "washout_window", "index_marker_gap", "combination_window") %in%
      colnames(cdm$output)
  ))
}
)
